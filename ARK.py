...
DiksFF = base64.b64decode(
    "QEFZQU5fQ0hFQVRTX09XTkVS"
).decode("utf-8")
...
def genkey(m):
    ...
    duration = args[1]
    key = "ARK-" + os.urandom(3).hex().upper()
...
def redeem(m):
    args = m.text.split()
    if len(args) < 2: return bot.reply_to(m, "Usage: /redeem ARK+123ABC")
